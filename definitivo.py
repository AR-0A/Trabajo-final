...
def backtracking(asignaturas, aulas, horario, horas_restantes):
    if not asignaturas:  # Caso base: todas las asignaturas procesadas
        return True

    asignatura = asignaturas[0]
    if horas_restantes[asignatura] == 0:  # Si la asignatura ya cumplió sus horas
        return backtracking(asignaturas[1:], aulas, horario, horas_restantes)

    # Ordenar aulas por capacidad
    ...
    for dia, horas in asignatura.profesor.disponibilidad.items():
        for hora in horas:
            # Calcular peso según preferencias
            ...
            for aula in aulas:
                if verificar_disponibilidad(horario, dia, hora, asignatura, aula):
                    ...
                    horas_restantes[asignatura] -= 1  # Reducir horas restantes

                    if backtracking(asignaturas, aulas, horario, horas_restantes):
                        return True

                    desasignar_horario(horario, dia, hora, asignatura, aula)
                    horas_restantes[asignatura] += 1  # Restaurar horas restantes

    return False

def verificar_disponibilidad(horario, dia, hora, asignatura, aula):
    # Verifica si el aula tiene capacidad suficiente
    if aula.capacidad < asignatura.tamaño_grupo:
        return False
...
def mostrar_horario_por_aula(horario, aulas):
    ...
    for aula in aulas:
        ...
        for dia in dias_semana:
            columnas.extend([dia.capitalize(), f"Profesor/Grupo {dia.capitalize()}"])
            ...
            for dia in dias_semana:
                if dia in horario and hora in horario[dia]:
                    ...
                    if clases:
                        ...
                        fila.append(f"{profesor.nombre}, {asignatura.grupo}")
                    else:
                        fila.extend(["", ""])
                else:
                    fila.extend(["", ""])
...
# Lista de asignaturas y diccionario de horas restantes
asignaturas = [asignatura1, asignatura2, asignatura3, asignatura4, asignatura5, asignatura6, asignatura7, asignatura8, asignatura9, asignatura10, asignatura11]
horas_restantes = {asignatura: asignatura.horas_semanales for asignatura in asignaturas}

# Generar el horario
horario = {}
if backtracking(asignaturas, [aula1, aula2, aula3], horario, horas_restantes):
